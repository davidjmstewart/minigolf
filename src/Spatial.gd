...
func mouse_rotation(event):
	...
	rot_y = clamp(rot_y,-PI/4, 0) # ensure that we only rotate about the X axis by an amount between -45 and 0 degrees(values determined by trial and error)
...
func _process(delta):
	...
	self.translation = ball_translation # set the position of our spatial node to the position of the ball
